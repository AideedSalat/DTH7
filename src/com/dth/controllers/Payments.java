...
public class Payments extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try{
            ...
            String selectTableSQL = "SELECT PAYMENTS FROM CUSTOMER_DETAILS WHERE EMAIL= '"+email+"'";
            
            rs = statement.executeQuery(selectTableSQL);
            rs.next();
            payments = rs.getInt("PAYMENTS");
            request.setAttribute("payments", payments);
            out.println(payments);
            request.getRequestDispatcher("WEB-INF/views/viewpayment.jsp").forward(request, response);
